...
" Save current working directory by Alan 2010.05.15
let s:SrcExpl_StartWorkingDir = getcwd()
...
" User interface to create a 'cscope.files' file using exact find 
" utility, the below as default by Alan 2010.05.15
   let g:SrcExpl_updateCscopeFilesCmd = "find . \\( -name '*.c' -o -name '*.cpp' -o -name '*.cc' -o -name '*.h' -o -name '*.s' -o -name '*.S' \\) -print > cscope.files"

" User interface to create a 'cscope.out' file using exact cscope 
" utility, the below as default by Alan 2010.05.15
   let g:SrcExpl_updateCscopeOutCmd = "cscope -b"
...
"
" Modified to use trinity plugin by Alan 2010.05.15
" SrcExpl_UpdateTags() do updating tags and cscope.out
" And Make those files in start working directory

function! g:SrcExpl_UpdateTags()

   " Save current working directory
   let s:TmpDir = getcwd()
   " Go to the start work directory
   silent! exe "cd " . s:SrcExpl_StartWorkingDir

   " Call the external 'ctags' utility program
   exe "!" . g:SrcExpl_updateTagsCmd

   exe "!" . g:SrcExpl_updateCscopeFilesCmd

   exe "!" . g:SrcExpl_updateCscopeOutCmd

   " Rejudge the tags file if existed
   if !filereadable("tags")
      " Tell them what happened
      echohl Question 
         \ | let tmpIn = <SID>SrcExpl_ConfirmStatus("Execute 'ctags' utility program failed\n"
              \ . "Press any key to continue...") |
      echohl None
      return -1
   endif

   " Rejudge the cscope.files file if existed
   if !filereadable("cscope.files")
      " Tell them what happened
      echohl Question 
         \ | let l:tmpIn = <SID>SrcExpl_ConfirmStatus("Execute 'find' utility program failed\n"
              \ . "Press any key to continue...") |
      echohl None
      return -1
   endif

   " Rejudge the cscope.out file if existed
   if !filereadable("cscope.out")
      " Tell them what happened
      echohl Question 
         \ | let l:tmpIn = <SID>SrcExpl_ConfirmStatus("Execute 'cscope' utility program failed\n"
              \ . "Press any key to continue...") |
      echohl None
      return -1
   endif

   " Tell them what command is successful
   echohl Question 
      \ | let l:tmpIn = <SID>SrcExpl_ConfirmStatus("\nUpdating tags & cscope.out files are successful\n"
              \ . "Press any key to continue...") |
   echohl None

   " Go back to the original work directory
   silent! exe "cd " . s:TmpDir
...
" SrcExpl_ConfirmStatus() {{{

" Output the message when we get an result and wait input

function! <SID>SrcExpl_ConfirmStatus(msg)

   " Be sure synchronize
   call inputsave()
   " Get the input content
   let l:input = input(a:msg)
   " Save the content
   call inputrestore()
...
function! <SID>SrcExpl_Init()
    ...
    " Added <CR><CR><CR> to skip prompt after run external command by Alan 2010.05.15
    if g:SrcExpl_updateTagsKey != ""
        exe "nnoremap " . g:SrcExpl_updateTagsKey . 
            \ " :call g:SrcExpl_UpdateTags()<CR><CR><CR><CR>"
    endif
